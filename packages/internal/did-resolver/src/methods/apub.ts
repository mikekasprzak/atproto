...
export type DidAPubMethodOptions = {
  fetch?: Fetch
  /** @default true */
  allowHttp?: boolean
}

export class DidAPubMethod implements DidMethod<'apub'> {
  ...
  constructor({
    ...
    allowHttp = true,
  }: DidAPubMethodOptions = {}) {
    this.fetch = bindFetch(fetch)
    this.allowHttp = allowHttp
  }

  async resolve(did: Did<'apub'>, options?: ResolveDidOptions) {
    const didDocumentUrl = buildDidAPubDocumentUrl(did)
 ...
 */
export function buildDidAPubDocumentUrl(did: Did<'apub'>) {
  const url = didAPubToUrl(did) // Will throw if the DID is invalid
  ...
  // TODO: return full webfinger lookup
  return `acct:${url}`
}
